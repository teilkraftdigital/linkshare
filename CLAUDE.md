...
**Linkshare** ist ein barrierefreies Link-Archiv — eine Laravel + Vue 3 SPA mit Single-User-Admin. Links werden mit Tags versehen; öffentliche Tags erzeugen teilbare URLs (`/tags/:slug`). Authentifizierung via Fortify (inkl. 2FA), kein Registrierungsformular.
...
# Development
composer run dev          # Concurrently runs PHP server, queue, log tail, and Vite
...
composer run lint          # Check PHP style (Pint)
vendor/bin/pint --dirty --format agent  # Fix modified PHP files
